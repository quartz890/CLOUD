...
export const LearnPage: React.FC<LearnPageProps> = ({
  ...
  return (
    <div id="learn-page-container" className="py-6 sm:py-16">
      <Helmet>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-xs font-medium text-slate-500 mb-4 sm:mb-8" aria-label="Breadcrumb">
          <button
            type="button"
        ...
        <div className="max-w-3xl mb-6 sm:mb-12">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-sky-50 border border-sky-100 text-[11px] sm:text-xs font-semibold text-sky-700 mb-3">
          ...
          <p className="mt-2 sm:mt-4 text-sm sm:text-lg text-slate-600 leading-relaxed font-normal">
        ...
        <div
          id="learn-filter-bar"
          className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-200 shadow-2xs mb-6 sm:mb-10 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4"
        >
          {/* Search Input */}
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search courses (e.g., HTML, React, Python)..."
              className="w-full pl-9 pr-3 py-2 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all"
            />
          </div>
        ...
        {filteredCourses.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-8 sm:p-12 text-center max-w-lg mx-auto">
            ...
            <button
              ...
              className="mt-4 px-4 py-2 rounded-xl text-xs font-semibold text-slate-900 bg-slate-100 hover:bg-slate-200 cursor-pointer"
            >
              Reset Filters
            </button>
          </div>
            ...
            {filteredCourses.map((course) => {
              ...
              return (
                <div
                  ...
                  className="bg-white rounded-2xl border border-slate-200 hover:border-slate-300 shadow-2xs hover:shadow-md transition-all duration-200 flex flex-col justify-between overflow-hidden group"
                >
                  {/* Card Body */}
                  <div className="p-4 sm:p-7">
                    ...
                    <div className="flex items-center justify-between gap-2 mb-4 sm:mb-5">
                      <div
                        className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center ${course.accentColor.bg} border ${course.accentColor.border}`}
                      >
                    ...
                    {/* Course Name */}
                    <h2 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight group-hover:text-sky-600 transition-colors">
                      {course.title}
                    </h2>
                    ...
                    {completed > 0 && (
                      <div className="mt-3.5 pt-3.5 sm:mt-4 sm:pt-4 border-t border-slate-100">
                    ...
                    <div className="mt-4 sm:mt-5 flex items-center gap-4 text-xs font-medium text-slate-500 pt-3 border-t border-slate-100">
                    ...
                    <div className="mt-3 sm:mt-4 flex flex-wrap gap-1.5">
                      {course.topics.slice(0, 3).map((topic, i) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-medium bg-slate-50 text-slate-600 border border-slate-200/60"
                        >
                          {topic}
                        </span>
                      ))}
                      {course.topics.length > 3 && (
                        <span className="px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-medium text-slate-400">
                  ...
                  <div className="p-4 pt-0 sm:p-6 sm:pt-0">
                    <button
                      ...
                      className={`w-full inline-flex items-center justify-center gap-2 py-2.5 sm:py-3 px-4 rounded-xl text-xs sm:text-sm font-semibold transition-all shadow-2xs hover:shadow-xs focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 active:scale-[0.99] cursor-pointer ${
                        isFullyCompleted
        ...
        <div className="mt-10 sm:mt-16 p-4 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
          ...
          <button
            ...
            className="w-full md:w-auto whitespace-nowrap px-5 py-2.5 rounded-xl text-xs sm:text-sm font-semibold text-slate-900 bg-slate-100 hover:bg-slate-200 transition-colors cursor-pointer text-center"
          >
            Start with HTML
          </button>
        </div>
